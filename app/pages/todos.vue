...
import { CheckIcon, CircleIcon, ClockIcon, Loader2Icon, TrashIcon } from '@lucide/vue'
...
<template>
  <div class="mx-auto w-full max-w-5xl px-4 py-6">
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
      <div>
        ...
        <p class="mt-1 text-sm text-muted-foreground">
          Action items generated by the AI for your legal matters.
        </p>
      </div>
    ...
    <div v-if="todoStore.loading || loadingConversations" class="flex items-center justify-center py-16">
      <Loader2Icon class="size-6 animate-spin text-muted-foreground" />
    </div>

    <div v-else-if="filteredTodos.length === 0" class="rounded-lg border border-dashed py-16 text-center">
      <p class="text-sm text-muted-foreground">No action items found.</p>
      <p class="mt-1 text-xs text-muted-foreground">
        Ask the AI to draft a legal document to generate next steps.
      </p>
    </div>
    ...
    <div v-else class="space-y-2">
      ...
      >
        <button
          ...
          class="mt-0.5 shrink-0 rounded-full p-1"
