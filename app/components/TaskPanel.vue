...
function priorityBadge(priority: Todo['priority']) {
  if (!priority) return ''
  return priority === 'high' ? 'bg-red-100 text-red-700 dark:bg-red-950 dark:text-red-400' : priority === 'medium' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-950 dark:text-yellow-400' : 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-400'
}
...
<template>
  <aside class="flex w-[340px] shrink-0 flex-col border-l bg-background">
      ...
      <div class="space-y-1.5 p-3">
        ...
        <template v-if="completedTasks.length > 0">
          ...
          >
            ...
            <button class="mt-0.5 shrink-0 cursor-pointer text-green-500" @click="toggleDone(todo)">
